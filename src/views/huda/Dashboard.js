...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    onSnapshot(
      collection(firestore, 'tahap1', 'huda', 'history'),
      (docSnap) => {
        let _data = [];
        docSnap.forEach((doc) => {
          _data.push(doc.data());
  ...
  return (
    <>
                ...
                <div className='flex flex-wrap'>
                  <div className='relative w-full pr-4 max-w-full flex-grow flex-1'>
                    ...
                    <span className='font-semibold text-xl text-blueGray-700'>
                      Jumlah Minyak Layak Digunakan
                    </span>
                  </div>
                </div>
                <div className='flex h-full justify-center items-center'>
                  <p
                    style={{ fontSize: '40pt', marginTop: '-30px' }}
              ...
              <div className='flex-auto p-4'>
                <div className='flex flex-wrap'>
                  <div className='relative w-full pr-4 max-w-full flex-grow flex-1'>
                    ...
                    <span className='font-semibold text-xl text-blueGray-700'>
                      Jumlah Minyak Tidak Layak Digunakan
                    </span>
                  </div>
                </div>
                <div className='flex h-full justify-center items-center'>
      ...
      <div className='my-6 flex flex-row items-center justify-center'>
        <p style={{ fontSize: '20pt', marginTop: '30px', fontWeight: 'bold' }}>
          Grafik Monitoring
        </p>
      </div>
      <div className='my-6 flex flex-row items-center justify-center'>
        <button
          onClick={() => onClick()}
          style={data?.deteksi ? style.buttonActive : style.buttonReset}
        >
          {data?.deteksi ? 'Start Deteksi' : 'Reset Deteksi'}
        </button>
      ...
      <div className='mt-4 flex flex-row items-center justify-center'>
        <div className='w-full xl:w-6/12 mb-12 xl:mb-0 px-4'>
          <HistoryChart
            name={source && mappingName[source]}
            source={source}
            data={dataHistory}
          />
        </div>
